...
import { catalogApi, Brand } from '@/lib/api-client';
...
export default function MarcasAdminPage() {
  ...
  const loadItems = async () => {
    if (!token) return;
    try {
      const { data } = await catalogApi.getBrands(token);
      setItems(data);
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await catalogApi.updateBrand(editing.id, formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Marca actualizada correctamente', type: 'success' });
      } else {
        await catalogApi.createBrand(formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Marca creada correctamente', type: 'success' });
      }
      setShowModal(false);
      loadItems();
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await catalogApi.deleteBrand(id, token);
      setToast({ isOpen: true, title: 'Éxito', message: 'Marca eliminada', type: 'success' });
      loadItems();
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Marcas de Vehículos</h1>
          <p className="text-slate-600 dark:text-slate-400">Gestiona las marcas disponibles</p>
        </div>
        <Button onClick={handleOpenCreate}><Plus className="w-5 h-5" /> Nueva Marca</Button>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{editing ? 'Editar Marca' : 'Nueva Marca'}</h2>
