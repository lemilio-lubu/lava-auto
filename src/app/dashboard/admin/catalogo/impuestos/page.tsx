...
import { catalogApi, TaxRate } from '@/lib/api-client';
import Button from '@/components/ui/Button';
import Modal from '@/components/ui/Modal';
import Toast from '@/components/ui/Toast';

type FormData = { name: string; percentage: string; isActive: boolean };

const defaultForm = (): FormData => ({ name: '', percentage: '', isActive: true });

export default function ImpuestosAdminPage() {
  ...
  const loadItems = async () => {
    if (!token) return;
    try {
      const { data } = await catalogApi.getTaxRates(token);
      setItems(data);
    } catch {
  ...
  const handleOpenEdit = (item: TaxRate) => {
    setEditing(item);
    setFormData({ name: item.name, percentage: item.percentage.toString(), isActive: item.isActive });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const payload = { name: formData.name, percentage: parseFloat(formData.percentage), isActive: formData.isActive };
      if (editing) {
        await catalogApi.updateTaxRate(editing.id, payload, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Tasa actualizada', type: 'success' });
      } else {
        await catalogApi.createTaxRate(payload, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Tasa creada', type: 'success' });
      }
      setShowModal(false);
      loadItems();
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!token) return;
    if (!confirm(`¿Eliminar la tasa "${name}"?`)) return;
    try {
      await catalogApi.deleteTaxRate(id, token);
      setToast({ isOpen: true, title: 'Éxito', message: 'Tasa eliminada', type: 'success' });
      loadItems();
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Tasas de Impuesto</h1>
          <p className="text-slate-600 dark:text-slate-400">Gestiona las tasas impositivas aplicables</p>
        </div>
        <Button onClick={handleOpenCreate}><Plus className="w-5 h-5" /> Nueva Tasa</Button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <th className="text-left px-6 py-3 font-semibold text-slate-600 dark:text-slate-300">Porcentaje</th>
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <td className="px-6 py-4 text-slate-700 dark:text-slate-300 font-semibold">{item.percentage}%</td>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{editing ? 'Editar Tasa' : 'Nueva Tasa de Impuesto'}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Porcentaje * (0–100)</label>
              <input
