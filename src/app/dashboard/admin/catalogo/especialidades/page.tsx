...
import { catalogApi, EmployeeSpecialty } from '@/lib/api-client';
...
export default function EspecialidadesAdminPage() {
  ...
  const [items, setItems] = useState<EmployeeSpecialty[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<EmployeeSpecialty | null>(null);
  ...
  const loadItems = async () => {
    if (!token) return;
    try {
      const { data } = await catalogApi.getEmployeeSpecialties(token);
      setItems(data);
    } catch {
  ...
  const handleOpenEdit = (item: EmployeeSpecialty) => {
    setEditing(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await catalogApi.updateEmployeeSpecialty(editing.id, formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Especialidad actualizada', type: 'success' });
      } else {
        await catalogApi.createEmployeeSpecialty(formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Especialidad creada', type: 'success' });
      }
      setShowModal(false);
      loadItems();
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await catalogApi.deleteEmployeeSpecialty(id, token);
      setToast({ isOpen: true, title: 'Éxito', message: 'Especialidad eliminada', type: 'success' });
      loadItems();
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Especialidades Técnicas</h1>
          <p className="text-slate-600 dark:text-slate-400">Gestiona las especialidades del personal</p>
        </div>
        <Button onClick={handleOpenCreate}><Plus className="w-5 h-5" /> Nueva Especialidad</Button>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{editing ? 'Editar Especialidad' : 'Nueva Especialidad'}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ej: Mecánica de motor"
              />
            </div>
