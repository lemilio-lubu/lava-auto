...
import { catalogApi, SparePartCategory } from '@/lib/api-client';
...
export default function CategoriasRepuestosAdminPage() {
  const { user, token, isLoading: authLoading } = useAuth();
  const router = useRouter();
  const [items, setItems] = useState<SparePartCategory[]>([]);
  ...
  const loadItems = async () => {
    if (!token) return;
    try {
      const { data } = await catalogApi.getSparePartCategories(token);
      setItems(data);
    } catch {
  ...
  const handleOpenEdit = (item: SparePartCategory) => {
    setEditing(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await catalogApi.updateSparePartCategory(editing.id, formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Categoría actualizada', type: 'success' });
      } else {
        await catalogApi.createSparePartCategory(formData, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Categoría creada', type: 'success' });
      }
      setShowModal(false);
      loadItems();
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await catalogApi.deleteSparePartCategory(id, token);
      setToast({ isOpen: true, title: 'Éxito', message: 'Categoría eliminada', type: 'success' });
      loadItems();
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Categorías de Repuestos</h1>
          <p className="text-slate-600 dark:text-slate-400">Gestiona las categorías de inventario</p>
        </div>
        <Button onClick={handleOpenCreate}><Plus className="w-5 h-5" /> Nueva Categoría</Button>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{editing ? 'Editar Categoría' : 'Nueva Categoría'}</h2>
