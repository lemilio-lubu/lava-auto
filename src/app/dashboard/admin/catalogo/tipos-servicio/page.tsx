...
import { catalogApi, ServiceType } from '@/lib/api-client';
import Button from '@/components/ui/Button';
import Modal from '@/components/ui/Modal';
import Toast from '@/components/ui/Toast';

type FormData = { name: string; description: string; isActive: boolean };

const defaultForm = (): FormData => ({ name: '', description: '', isActive: true });

export default function TiposServicioAdminPage() {
  ...
  const loadItems = async () => {
    if (!token) return;
    try {
      const { data } = await catalogApi.getServiceTypes(token);
      setItems(data);
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const payload = { name: formData.name, description: formData.description || undefined, isActive: formData.isActive };
      if (editing) {
        await catalogApi.updateServiceType(editing.id, payload, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Tipo de servicio actualizado', type: 'success' });
      } else {
        await catalogApi.createServiceType(payload, token);
        setToast({ isOpen: true, title: 'Éxito', message: 'Tipo de servicio creado', type: 'success' });
      }
      setShowModal(false);
      loadItems();
    } catch (error: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!token) return;
    if (!confirm(`¿Eliminar el tipo de servicio "${name}"?`)) return;
    try {
      await catalogApi.deleteServiceType(id, token);
      setToast({ isOpen: true, title: 'Éxito', message: 'Tipo de servicio eliminado', type: 'success' });
      loadItems();
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Tipos de Servicio</h1>
          <p className="text-slate-600 dark:text-slate-400">Gestiona los servicios del taller</p>
        </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{editing ? 'Editar Tipo de Servicio' : 'Nuevo Tipo de Servicio'}</h2>
